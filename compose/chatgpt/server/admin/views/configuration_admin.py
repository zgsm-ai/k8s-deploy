...
class ConfigurationAdmin(AdminPermission, BaseView):
    ...
    def after_model_change(self, form, model, is_created=False):
        # 更新后清理广告配置缓存
        if model.belong_type == ConfigurationConstant.QIANLIU_AD_TYPE:
            ConfigurationService.evict_ad_cache()
        elif model.belong_type == ConfigurationConstant.ZHUGE_ADS_TYPE:
            ConfigurationService.evict_zhuge_ads_cache()
        elif model.attribute_key == ConfigurationConstant.LANGUAGE_KEY_MAP:
