...
from common.constant import UserConstant, AnalysisConstant, ConfigurationConstant, CodeCompletionConstant
...
class UsersService(BaseService):
    ...
    def create_eolinker_user(cls, username, display_name):
        """
        创建eolinker测试平台用户，API接口测试生成使用
        """
        avatar_color = generate_random_avatar_color()
        search_kw = {
            'username': username,
            'display_name': display_name,
            'description': "eolinker"
        }
        defaults = {
            **search_kw,
            'avatar_color': avatar_color
    ...
    @classmethod
    def get_code_completion_allow_department(cls):
        work_id = g.current_user.username
        multilevel_dept = analysis_service.get_user_multilevel_dept(work_id)  # 多级部门信息

        allow_departments = ConfigurationService.get_configuration_with_cache(
            ConfigurationConstant.CODE_COMPLETION_TYPE,
            CodeCompletionConstant.allow_departments,
            ''
        )
        return multilevel_dept, allow_departments

    @classmethod
    def get_need_refusal_cause(cls):
        user = g.current_user
        multilevel_dept = analysis_service.get_user_multilevel_dept(work_id=user.username)
        dept_result = ConfigurationService.get_configuration_with_cache(ConfigurationConstant.PERMISSION_TYPE,
                                                                        ConfigurationConstant.SCRIBE_NEED_CAUSE_DEPT,
                                                                        '')
        dept_list = dept_result.split(',') if dept_result else []
        for dept in dept_list:
            if multilevel_dept.startswith(dept):
                return {'need_refusal_cause': True}
        return {'need_refusal_cause': False}
