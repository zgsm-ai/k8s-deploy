...
class AgentChatBot(ChatBotWithHistory):
    ...
    def chat_stream(self, req: ChatRequestData, user_context: str, mode="normal", user=None):
        """
        流式对话，根据模式与智能团对话，或者和单角色对话
        """
        ...
        if mode == "group":
            self._chat_group(req, user_context, user=user)
        else:
            self._chat_normal(req, user_context, user=user)
        self.save_conversation(self.conv_id)

    def _chat_group(self, req: ChatRequestData, user_context: dict, user=None):
        """
        ...
        self.chat_group_data = [
            available_members, group_guardian, observer_agent, user_context, req.prompt, user
        ]
        ...
        try:
            user_proxy.initiate_chat(manager, message=req.prompt, clear_history=False)
    ...
    def _get_advise(self, req: ChatRequestData, user_context: dict, user=None):
        """
        ...
        options.stream = True
        strategy_advise = get_action_strategy(data.action)
        data.prompt = strategy_advise.get_prompt(req)
        result = strategy_advise.make_result(data, options)
    ...
    def _chat_normal(self, req: ChatRequestData, user_context: dict, user=None):
        """
        普通对话，无代码上下文
        ...
        logger.info(f"chat_normal(): data: {json.dumps(data.__dict__)}")
        logger.info(f"chat_normal(): options: {json.dumps(options.__dict__)}")
        logger.info(f"chat_normal(): strategy: {strategy.name}")
        full_data = ""
        ...
        self._get_advise(req, user_context, user)
