...
class ActionStrategy:
    ...
    def make_chatbot(self, data: ChatRequestData = None, options: ChatbotOptions = None):
        ...
        return apiBot(history=history, prompter=prompter, model=model)
    ...
    def ask(self, data: ChatRequestData, options: ChatbotOptions = None):
        ...
        logger.info(f"ActionStrategy.ask(): prompt: {data.prompt}")
        logger.info(f"ActionStrategy.ask(): data: {data.__dict__}")
        logger.info(f"ActionStrategy.ask(): options: {options.__dict__}")
        if options.stream:
