...
def code_completion_log():
    ...
    if data.get("action") == ActionsConstant.SCRIBE:
        ...
        AIRecordActionService.update_with_accept(data)
    elif accept_key in data.keys():
...
@users.route('/code_completion/allow_department', methods=['GET'])
def check_allow_department():
    """
    可接受部门
    ---
    tags:
    ...
    responses:
      200:
        res: 结果
    """
    multilevel_dept, allow_departments = users_service.get_code_completion_allow_department()
    resp = {
        'multilevel_dept': multilevel_dept,
        'allow_departments': allow_departments
    }
    return Result.success(message='获取成功', data=resp)


@users.route('/need_refusal_cause', methods=['GET'])
def check_need_refusal_cause():
    """
    用户是否需要拒绝原因
    ---
    tags:
    ...
    responses:
      200:
        res: 结果
    """
    resp = users_service.get_need_refusal_cause()
    return Result.success(message='获取成功', data=resp)
