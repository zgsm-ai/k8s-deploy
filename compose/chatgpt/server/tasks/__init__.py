...
celery_app.conf.update(
    beat_schedule={
        'calibration_review_data_task': {
            'task': 'tasks.beat_task.calibration_review_data_task',
            'schedule': CONFIG.app.get('CELERY_SCHEDULE', {}).get('calibration_review_data_task', 300),
            'args': ()
        },
        'calibration_api_test_case_task': {
            'task': 'tasks.api_test_case_task.check_api_test_eolinker_case_task',
            'schedule': CONFIG.app.get('CELERY_SCHEDULE', {}).get('calibration_api_test_case_task', 180),
            'args': ()
        },
