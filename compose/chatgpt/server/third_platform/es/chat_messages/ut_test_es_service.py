...
from third_platform.es.base_es import UT_CASE_INDEX, DOC, BaseESService
from third_platform.services.analysis_service import analysis_service


class UTCaseESservice(BaseESService):
    """用例统计到es"""
    logger = logging.getLogger(__name__)

    def __init__(self):
        super(UTCaseESservice, self).__init__()
        self.index = UT_CASE_INDEX
    ...
    def insert_code_completion(self, data):
        ...
        try:
            ...
            code_md5 = self.calculate_md5(data['display_name'] + str(data['timestamp']))
            obj_dict = {
                "id": code_md5,
                ...
                "cur_file_count": data.get("cur_file_count"),
                "cur_method_count": data.get("cur_method_count"),
                "generated_case_count": data.get("generated_case_count")
            }
...
ut_case_es_service = UTCaseESservice()
