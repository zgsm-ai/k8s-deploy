...
class Config:
    ...
    def __init__(self, env_name='FLASK_ENV', default_env='development', local_config_filename='local.yml'):
        ...
        if self.env == 'test':
            self.local_config_filename = 'test.yml'
        else:
            self.local_config_filename = local_config_filename
    ...
    def load(self):
        for config_file in os.listdir(self.runtime_path):
            if config_file[-4:] == '.yml' and config_file != self.local_config_filename:
                config_name = config_file[0:-4]
                cfg = self._load_file(config_file)
    ...
    def _load_local(self):
        self.update(self._load_file(self.local_config_filename))
    ...
    def _load_file(self, filename):
        ...
        with open(path, 'r', encoding='utf-8') as stream:
            try:
                ...
                if cfg is None:
                    return {}
                if filename == self.local_config_filename or filename[0:-4] in self.buildin_config:
                    return cfg
                else:
...
def get_config():
    return {
        "duckduck_api_base": os.environ.get("DUCKDUCK_API_BASE", "http://10.8.0.120:7000"),
        ...
        "auth_url": os.environ.get("AUTH_URL", "https://idtrust.atrust.sangfor.com"),
        "flask_config": {
            "SECRET_KEY": "ai"
        },
        "jwt": {
        ...
        "redirect_url": os.environ.get("REDIRECT_URL", "http://devops-test.sangfor.com"),
        ...
        "review_url": os.environ.get("REVIEW_URL", "https://chatgpt.sangfor.com"),
        "qdrant_server": os.environ.get("QDRANT_SERVER"),
        "APP_URL": os.environ.get("APP_URL", "https://chat.sangfor.com"),
