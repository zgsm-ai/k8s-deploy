...
AD_ADDR="http://172.16.254.5:32081/v1/completions"
FILE="./access_token"
...
echo FAUXPILOT: ----------------------------------------------------------------
time curl -k -i $FAUX_ADDR -H "$AUTH" -H "$TYPE" -X POST  -d "$DATA"

echo AD: -----------------------------------------------------------------------
time curl -i $AD_ADDR -H "$TYPE" -X POST  -d "$DATA"

PORTS=(32320 32321 32324)

for port in "${PORTS[@]}"; do
  echo -------------------------------------------------------------------------
  echo curl -i http://172.16.254.5:$port/v1/completions -H "$TYPE" -X POST  -d "$DATA"
  time curl -i http://172.16.254.5:$port/v1/completions -H "$TYPE" -X POST  -d "$DATA"
done
